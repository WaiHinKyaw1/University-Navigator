...
import { BrainCircuit, CheckCircle2, ChevronDown, ChevronUp, GitCompare, Route, Target } from "lucide-react";
...
export default function InterestGuide() {
  ...
  return <Layout><main className="mx-auto w-full max-w-6xl space-y-8 px-4 py-10 scrollbar-hide">
    <header className="text-center"><div className="mx-auto mb-3 flex h-14 w-14 items-center justify-center rounded-2xl bg-primary/10 text-primary"><BrainCircuit /></div><h1 className="text-3xl font-bold md:text-4xl">Career Interest Guide</h1></header>

    {!analysis ? <Card className="mx-auto max-w-4xl border-primary/15 shadow-lg"><CardHeader><CardTitle>သင်သိချင်တာကို လွတ်လပ်စွာ မေးပါ</CardTitle><p className="text-sm text-muted-foreground">သင့်အကြောင်းနဲ့ ဖြစ်ချင်တဲ့အလုပ်ကို စာကြောင်းအပြည့် ရေးပါ။</p></CardHeader><CardContent className="grid gap-5"><Field label="သင့်မေးခွန်း / သင့်အကြောင်း" placeholder="ကွန်ပျူတာနဲ့ website တည်ဆောက်ရတာကြိုက်တယ်၊ programming လေ့လာချင်တယ်။ ဘာအလုပ်နဲ့ သင့်တော်မလဲ?" value={question} setValue={setQuestion} />
      ...
      <Button className="h-12 text-base md:col-span-2" onClick={analyze} disabled={loading || [question, skills, interests, workPreferences, careerGoals, experience].join(" ").trim().length < 5}>{loading ? "သင့်အကြောင်းကို စစ်ဆေးနေသည်..." : "သင့်အတွက် Career ရှာမည်"}</Button>
    </CardContent></Card> : <>
...
function Comparison({ careers }: { careers: CareerResult[] }) { return <Card className="border-primary/20"><CardHeader><CardTitle className="flex items-center gap-2"><GitCompare className="h-5 w-5 text-primary" />Career Comparison</CardTitle><p className="text-sm text-muted-foreground">Career နှစ်ခု သို့မဟုတ် သုံးခုကို နှိုင်းယှဉ်ကြည့်ပါ။</p></CardHeader><CardContent className="overflow-x-auto"><div className="grid min-w-[720px] gap-3" style={{ gridTemplateColumns: `repeat(${careers.length}, minmax(0, 1fr))` }}>{careers.map((item) => <div key={item.career.id} className="rounded-lg bg-muted/40 p-4"><h3 className="font-bold">{item.career.title}</h3><div className="my-2 text-2xl font-black text-primary">{item.score}%</div><p className="text-sm">Skill gaps: {item.skillGaps.length}</p><p className="text-sm">Roadmap: {item.career.roadmap.length} steps</p><p className="mt-2 text-xs text-muted-foreground">{item.career.workPreferences.join(" • ")}</p></div>)}</div></CardContent></Card>; }
