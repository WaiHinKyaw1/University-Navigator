...
export default function InterestGuide() {
  ...
  return <Layout><main className="mx-auto w-full max-w-6xl space-y-8 px-4 py-10 scrollbar-hide">
    ...
    {!analysis ? <Card className="mx-auto max-w-4xl border-primary/15 shadow-lg"><CardHeader><CardTitle>သင်သိချင်တာကို လွတ်လပ်စွာ မေးပါ</CardTitle><p className="text-sm text-muted-foreground">သတ်မှတ်ထားတဲ့ စကားလုံးတွေကိုပဲ ရေးစရာမလိုပါဘူး။ မိမိအကြောင်းနဲ့ ဖြစ်ချင်တဲ့အလုပ်ကို စာကြောင်းအပြည့် ရေးနိုင်ပါတယ်။</p></CardHeader><CardContent className="grid gap-5 md:grid-cols-2"><div className="md:col-span-2"><Field label="သင့်မေးခွန်း / သင့်အကြောင်း" placeholder="ဥပမာ - ကျွန်တော်က လူတွေနဲ့ အလုပ်လုပ်ရတာကြိုက်တယ်၊ အားကစားဝါသနာပါတယ်။ ဘောလုံးသမား ဖြစ်ချင်တယ်။ ဘာ Career နဲ့ သင့်တော်မလဲ?" value={question} setValue={setQuestion} /></div>
      ...
      <Button className="h-12 text-base md:col-span-2" onClick={analyze} disabled={loading || [question, skills, interests, workPreferences, careerGoals, experience].join(" ").trim().length < 5}>{loading ? "NLP ခွဲခြမ်းစိတ်ဖြာနေသည်..." : "Career Recommendation ရှာမည်"}</Button>
    </CardContent></Card> : <>
