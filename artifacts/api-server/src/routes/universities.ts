...
router.get("/universities/:id", optionalAuth, async (req, res): Promise<void> => {
...
router.post("/universities", requireAdmin, async (req, res): Promise<void> => {
  const { name, nameEn, abbreviation, type, state, city, minScore, description, website, imageUrl, majorIds } = req.body;
  ...
  const [uni] = await db
    ...
    .values({ name, nameEn, abbreviation, type, state, city, minScore, description, website, imageUrl })
    .returning();
...
router.put("/universities/:id", requireAdmin, async (req, res): Promise<void> => {
  const raw = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
  const id = parseInt(raw, 10);
  if (isNaN(id)) {
    res.status(400).json({ error: "Invalid id" });
  ...
  const [updated] = await db
    ...
    .set({ name, nameEn, abbreviation, type, state, city, minScore, description, website, imageUrl })
  ...
  if (majorIds && Array.isArray(majorIds)) {
    await db.delete(universityMajorsTable).where(eq(universityMajorsTable.universityId, id));
...
router.delete("/universities/:id", requireAdmin, async (req, res): Promise<void> => {
  const raw = Array.isArray(req.params.id) ? req.params.id[0] : req.params.id;
  const id = parseInt(raw, 10);
  if (isNaN(id)) {
    res.status(400).json({ error: "Invalid id" });
  ...
  const [deleted] = await db.delete(universitiesTable).where(eq(universitiesTable.id, id)).returning();
  if (!deleted) {
    res.status(404).json({ error: "University not found" });
    return;
  }
